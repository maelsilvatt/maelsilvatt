...
* **[OmniScribe AI – Regulatory Audit & Compliance Engine](https://github.com/maelsilvatt/omniscribe-ai-regulatory-audit-engine)**: A scalable Retrieval-Augmented Generation (RAG) architecture engineered to ingest raw corporate contracts audit it against complex regulatory frameworks (LGPD, Central Bank norms, ISOs). Features an asynchronous Python processing pipeline utilizing `asyncio` and `FastAPI` for low-latency context retrieval, leveraging ChromaDB for semantic compliance mapping and the Google Gemini API for context-aware, deterministic risk synthesis and automated reporting.
    * `Tech: Python | Data Engineering | LLM | FastAPI | Redis | PostgreSQL | Google Gemini API | ChromaDB | Asyncio | Vector Embeddings | Docker`

* **[ReaperVS-Generator – Automated .RPP Project Generator for Musicians](https://github.com/maelsilvatt/reaper-vs-generator)**: A professional automation tool that generates Reaper project files (.RPP) complete with stems via Demucs 6s, transient-analysis click tracks, and Split Mono routing from audio tracks, chord sheets, or web URLs. Features a robust data engineering pipeline that utilizes LLMs and Regex to clean and normalize non-standard chord sheets, powering automated guide vocal generation via TTS models and faster-whisper alignment. Designed around a Hexagonal Architecture that allows for proprietary web scrapers, it features a containerized producer-consumer system that integrates with Telegram for direct bot interactions and automated setlist generation using a deep-link database. Includes a production-ready deployment guide for personal environments.
   * `Tech: Python | Data Engineering | LLM | faster-whisper | TTS Models | Demucs 6s (Meta) | Reaper (.RPP) | Redis | TursoDB (SQLite) | Telegram Bot API | Docker | Hexagonal Architecture`
...
* **[Stratflow Sim Lab – Agent Benchmarking & Logistics Platform](https://github.com/nitsuboy/Simulador_Grafos)**: An event oriented platform built to evaluate AI agents tackling classic logistics and network graph constraints (max flow, route optimization). Features high-performance Python RESTful APIs and a highly concurrent state persistence orchestration layer powered by **Go (Goroutines)** for real-time request pairing.
